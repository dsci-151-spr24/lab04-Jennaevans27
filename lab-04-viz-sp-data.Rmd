...
***There are `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents the data for any denny's restaurants. The variables are: `r names(dennys)`.*** 

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?

***There are `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents the data for any La Quinta's restaurants. The variables are: `r names(laquinta)`.*** 
...
***La Quinta hotels are mostly located in the Americas but also have locations in other parts of the world. Denny's are only located in the United States.*** 
...
***We can use the state variable to determine whether a location is in the US Specifically by comparing the state listed to a database of state listed to a database of states in the US.***
...
***There are no denny's location outside of the US in our data.***
...
***Delware has the fewest Denny's, well California has the most. Maine has the fewest La Quinta's and Texas has the most. No this is not suprising becuase the states with the fewest are small and the ones with the most are big.*** 
...
DC have the most per thousand square miles of Denny's, along with RI, CA and CT. RI has the most La Quinta's per thousand miles along with FL, CT, and MD. 
